...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="mb-8 rounded-xl overflow-hidden">
        <img src="{{ asset('images/mantenimientoedi.png') }}" alt="Banner Mantenimiento"
        ...
        <div class="max-w-4xl mx-auto">
            <div class="inline-block bg-gray-100/80 px-6 py-3 rounded-lg mb-6 backdrop-blur-sm border border-gray-200">
                <p class="text-lg font-medium text-gray-700">
                    Panel de información de las tareas de la <strong>Dirección de Programación y Mantenimiento Edilicio</strong> sobre el mantenimiento en los establecimientos educativos.
                </p>
            </div>
        </div>
    </div>
                    ...
                    @forelse(($registros['APH'] ?? []) as $r)
                        <div class="border-b border-gray-100 last:border-0 py-3">
                            <p class="font-medium text-gray-800">
                                <span class="text-purple-600">{{ \Carbon\Carbon::parse($r->fecha)->format('d/m') }}</span> -
                                {{ $r->establecimiento }}
                            </p>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div class="p-4 {{ $scrollELEC ? 'max-h-96 overflow-y-auto pr-1' : '' }}">
                    @forelse(($registros['ELEC'] ?? []) as $r)
                        <div class="border-b border-gray-100 last:border-0 py-3">
                            <p class="font-medium text-gray-800">
                                <span class="text-blue-600">{{ \Carbon\Carbon::parse($r->fecha)->format('d/m') }}</span> -
                                {{ $r->establecimiento }}
                            </p>
                            <p class="text-gray-600 text-sm mt-1">{{ $r->tarea }}</p>
                        </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div class="p-4 {{ $scrollDEZM ? 'max-h-96 overflow-y-auto pr-1' : '' }}">
                    @forelse(($registros['DEZM'] ?? []) as $r)
                        <div class="border-b border-gray-100 last:border-0 py-3">
                            <p class="font-medium text-gray-800">
                                <span class="text-green-600">{{ \Carbon\Carbon::parse($r->fecha)->format('d/m') }}</span> -
                                {{ $r->establecimiento }}
                            </p>
                            <p class="text-gray-600 text-sm mt-1">{{ $r->tarea }}</p>
                        </div>
            ...
            <div class="w-full overflow-x-auto {{ $maxHeight }} overflow-y-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="px-6 py-8 text-center text-gray-500 italic">Sin pendientes</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="w-full overflow-x-auto {{ $maxHeight }} overflow-y-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                                ...
                                <td class="px-4 py-3">
                                    @php
                                        $estadoColor = match(strtolower($c->estado)) {
                                            'completado', 'finalizado' => 'bg-green-100 text-green-800 ring-green-200',
                                            'en proceso', 'en progreso' => 'bg-blue-100 text-blue-800 ring-blue-200',
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="px-6 py-8 text-center text-gray-500 italic">Sin comisiones</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
...
<script>
  (function () {
    ...
    form.addEventListener('submit', function () {
      try { sessionStorage.setItem('edudata_mant_scrollY', String(window.scrollY || window.pageYOffset || 0)); } catch (e) {}
    });
